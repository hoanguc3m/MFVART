#' Bayesian inference of VAR model with factor covariance matrix
#'
#' Bayesian inference of VAR model with factor covariance matrix
...
#' @examples
#' \dontrun{
#' datagen <- sim.BVAR.novol(dist="Gaussian", idq = c(4), p = 2, K = 4, t_max = 500)
#' y <- datagen$y
#' prior <- get_prior(y, p = 2, dist="Gaussian", SV = F)
#' inits <- get_init(prior)
#' Chain1 <- JCVARF.novol(y, K = 5, p = 2, dist = "Gaussian", y0 = NULL, prior = prior, inits = inits)
#' plot(Chain1)
#' }
JCVARF.novol <- function(y, K, p, dist, y0 = NULL, prior = NULL, inits = NULL){
  if (!(dist %in% c("Gaussian","Student","OT") ))
    stop("dist is not implemented.")
  
  if (prior$SV == FALSE){
    Start = Sys.time()
    if (dist == "Gaussian") Chain <- JCVARF.Gaussian.novol(y, K, p, y0, prior, inits)
    if (dist == "Student") Chain <- JCVARF.Student.novol(y, K, p, y0, prior, inits)
    if (dist == "OT") Chain <- JCVARF.OT.novol(y, K, p, y0, prior, inits)
...
###########################################################################
#' @export
JCVARF.Gaussian.novol <- function(y, K, p, y0 = NULL, prior = NULL, inits = NULL){
...
###########################################################################
#' @export
JCVARF.Student.novol <- function(y, K, p, y0 = NULL, prior = NULL, inits = NULL){
  # Init regressors in the right hand side
  t_max <- nrow(y)
  yt = t(y)
  xt <- makeRegressor(y, y0, t_max, K, p)
  ...
  for (j in c(1:samples)){
    ...
    e = matrix((yt - B %*%xt),ncol = 1)
    Xf <- kronecker(Diagonal(t_max), L)
    sigw <- rep(1/sigma2, t_max) * rep(1/w_sample, each = K)
    XfiSig <- Matrix::t(Xf) %*% Diagonal(t_max*K, sigw )
    Kf <- Diagonal(t_max*r, 1/sigmaf2) + XfiSig %*% Xf
    CKf <- chol(Kf)
    f <- backsolve( CKf,
                    backsolve( CKf, XfiSig %*% e,
    ...
    for (ii in 1:K) {
      if (ii <= r) {
        ...
        iVthetai <- Diagonal(x = 1/c(V_b_prior[ii,], 
                                     rep(1 / Vlhyper, ii-1)))
        thetai0 <- c(b_prior[ii,], rep(mulhyper, ii-1))
        ZiSigi <- Zi %*% Diagonal(t_max, 1/sigma2[ii] / w_sample)
        dthetai <- iVthetai %*% thetai0 + ZiSigi %*% (yt[ii,] - Fmat[ii,])
      } else {
        Zi <- rbind(xt, Fmat)
        iVthetai <- Diagonal(x = 1/c(V_b_prior[ii,], rep(Vlhyper,r)))
    ...
    # Sample sigma
    u_ort <- (yt - B %*%xt - L %*% Fmat)/ w_sqrt    # change from Gaussian
    sigma_post_a <- sigma0_T0 + rep(t_max,K)
    sigma_post_b <- sigma0_S0 + rowSums(u_ort^2)
    for (i in c(1:K)){
      sigma2[i] <- rinvgamma(1, shape = sigma_post_a[i] * 0.5, rate = sigma_post_b[i] * 0.5)
...
###########################################################################
#' @export
JCVARF.OT.novol <- function(y, K, p, y0 = NULL, prior = NULL, inits = NULL){
  # Init regressors in the right hand side
  t_max <- nrow(y)
  yt = t(y)
  xt <- makeRegressor(y, y0, t_max, K, p)
  ...
  for (j in c(1:samples)){
    ...
    e = matrix((yt - B %*%xt),ncol = 1)
    Xf <- kronecker(Diagonal(t_max), L)
    sigw <- rep(1/sigma2, t_max) * as.numeric(1/w)
    XfiSig <- Matrix::t(Xf) %*% Diagonal(t_max*K, sigw )
    Kf <- Diagonal(t_max*r, 1/sigmaf2) + XfiSig %*% Xf
    CKf <- chol(Kf)
    f <- backsolve( CKf,
